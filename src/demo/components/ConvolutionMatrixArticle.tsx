...
const borderCode = `// 扩展（Extend）：复制最靠近的边缘纹理像素（Texel）
...
// 裁剪（Crop）：在着色器（Shader）中检测边缘片段，直接输出中心样本。`;
  ...
  return (
    <article className="lesson-article">
      <header className="lesson-hero">
        ...
        <p className="lesson-lead">沿用 GIMP 卷积矩阵（Convolution Matrix）的参数模型，把卷积核（Kernel）、除数（Divisor）、偏移量（Offset）、边缘处理（Border）、通道（Channels）、归一化（Normalize）与 Alpha 加权（Alpha weighting）映射到 WebGL2 纹理采样。</p>
        <ul className="lesson-meta" aria-label="课程信息"><li>卷积矩阵（Convolution Matrix）</li><li>图像滤镜（Image Filters）</li><li>约 28 分钟</li></ul>
      </header>
      ...
      <section className="learning-note" aria-labelledby="convolution-matrix-learning">
        ...
        <div><h2 id="convolution-matrix-learning">完成这一节后</h2><ul><li>能把图像理解成按矩形坐标排列的像素矩阵</li><li>能手算一个 3×3 卷积核（Kernel）的输出</li><li>理解除数（Divisor）与偏移量（Offset）如何调节结果范围</li><li>能比较扩展（Extend）、环绕（Wrap）、裁剪（Crop）三种边缘策略</li><li>知道通道选择、归一化（Normalize）与 Alpha 加权（Alpha weighting）的用途</li></ul></div>
      </section>

      <section id="image-and-kernel" className="lesson-section">
        <h2>图像矩阵遇到卷积核（Kernel）</h2>
        <p>图像可以看作二维像素集合，每个像素位于明确的 X、Y 坐标。卷积处理依次访问输出图像的每个位置，以当前像素为中心读取一块邻域，再让邻域数值与卷积核（Kernel）中对应位置的权重相乘并求和。</p>
        ...
        <div className="convolution-overview" aria-label="图像邻域经过卷积矩阵得到输出像素">
          <span><Grid3X3 aria-hidden="true" /><strong>3×3 像素邻域</strong><small>九个输入颜色</small></span><i>×</i><span><Sigma aria-hidden="true" /><strong>3×3 卷积核（Kernel）</strong><small>九个权重</small></span><i>→</i><span><Palette aria-hidden="true" /><strong>输出像素</strong><small>加权和经过调整</small></span>
        </div>
      </section>

      <section id="one-pixel-example" className="lesson-section">
        <h2>手算一个输出像素</h2>
        <p>GIMP 文档使用一组灰度数值演示计算。卷积核（Kernel）只有上方中间一项为 1，所以输出会直接取得当前像素上方的值 42。对整张图执行后，每个位置都读取它的上方邻居，视觉内容会整体下移一个像素。</p>
        <CodeBlock label="convolution-example.ts">{gimpExampleCode}</CodeBlock>
      </section>

      <section id="divisor-and-offset" className="lesson-section">
        <h2>除数（Divisor）与偏移量（Offset）调整加权和</h2>
        <p><strong>除数（Divisor）</strong> 用于缩放加权和。九个权重全为 1 的均值模糊会产生九项总和，除以 9 后恢复平均亮度。<strong>偏移量（Offset）</strong> 在除法后相加，适合把负结果移回可显示范围；例如边缘检测常配合 128，让正负变化分别落到中灰两侧。</p>
        <CodeBlock label="卷积结果公式">{resultFormula}</CodeBlock>
        <div className="convolution-option-cards"><article><Divide aria-hidden="true" /><h3>除数（Divisor）= 9</h3><p>九个邻居取平均，整体亮度保持稳定。</p></article><article><Orbit aria-hidden="true" /><h3>偏移量（Offset）= 128</h3><p>零响应显示为中灰，负值和正值仍可观察。</p></article></div>
      </section>

      <section id="border-modes" className="lesson-section">
        <h2>边缘处理（Border）决定图像范围外从哪里取值</h2>
        <p>中心像素靠近图像边缘时，卷积核（Kernel）的一部分会落在纹理范围外。实验提供三种策略，并用 WebGL2 的纹理环绕状态或着色器（Shader）分支实现。</p>
        <dl className="border-mode-list"><div><dt>扩展（Extend）</dt><dd>使用最靠近的边缘纹理像素（Texel），映射到 <code>CLAMP_TO_EDGE</code>。</dd></div><div><dt>环绕（Wrap）</dt><dd>从对边继续读取，映射到 <code>REPEAT</code>。</dd></div><div><dt>裁剪（Crop）</dt><dd>边缘片段保留中心样本，只处理拥有完整 3×3 邻域的位置。</dd></div></dl>
        <CodeBlock label="texture-border.ts">{borderCode}</CodeBlock>
      </section>

      <section id="channels-and-alpha" className="lesson-section">
        <h2>通道（Channels）与 Alpha 加权（Alpha weighting）</h2>
        <p>通道（Channels）选择决定卷积结果写回红、绿、蓝、透明度（RGBA）中的哪些分量。未选中的通道沿用中心样本。普通彩色滤镜通常处理 RGB 并保留原始透明度（Alpha）；需要改变透明轮廓时再启用 A。</p>
        <p>Alpha 加权（Alpha weighting）用于透明图像。透明像素仍可能保存 RGB，直接模糊容易把隐藏颜色带入边缘。处理时先让 RGB 乘以透明度（Alpha），卷积完成后再按输出透明度（Alpha）恢复颜色，可以减少暗边或彩边。</p>
      ...
      <section id="normalise" className="lesson-section">
        <h2>归一化（Normalize）自动推导结果调整</h2>
        <p>启用归一化（Normalize）后，正权重和直接成为除数（Divisor）。权重和为零时无法执行除法，实验使用除数（Divisor）1 与偏移量（Offset）128；权重和为负数时使用其绝对值作为除数（Divisor），并设置偏移量（Offset）255。这套行为对应 GIMP 文档描述的自动范围修正。</p>
      </section>

      <section id="convolution-matrix-lab" className="lesson-section lesson-section--wide">
        <h2>完整卷积矩阵实验</h2>
        <p>先选择“下移一像素”复现文档示例，再切换锐化、边缘检测与浮雕。你也可以直接编辑九个权重，控制除数（Divisor）、偏移量（Offset）、边缘处理（Border）和 RGBA 通道。输入图像由站内画布（Canvas）生成，参数变化后会重新执行一次 WebGL2 绘制调用（Draw Call）。</p>
        <ConvolutionMatrixPlayground />
      </section>

      <section id="webgl-mapping" className="lesson-section">
        <h2>在片段着色器中映射这些选项</h2>
        <p><code>textureSize</code> 给出纹理尺寸，它的倒数就是相邻纹理像素（Texel）的 UV 步长。九个样本乘以 <code>u_kernel[9]</code>，随后应用 <code>u_divisor</code> 和 <code>u_offset</code>。<code>u_channelMask</code> 决定保留中心样本还是写入卷积结果。</p>
